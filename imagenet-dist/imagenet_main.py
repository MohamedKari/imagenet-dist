...
r"""
Adapted Source: `pytorch imagenet example <https://github.com/pytorch/examples/blob/master/imagenet/main.py>`_ # noqa B950
Changes for TorchElastic:
...
Changes by Mo:
6. Remove all TorchElastic references again
...
parser.add_argument("-a", "--arch", default="resnet18", choices=model_names, 
...
parser.add_argument("-b", "--batch-size", default=32, type=int, help="mini-batch size (default: 32), per worker (GPU)")
...
def initialize_model(arch: str, lr: float, momentum: float, weight_decay: float, device_id: int):
...
def initialize_data_loader(data_dir, batch_size, num_data_workers) -> Tuple[DataLoader, DataLoader]:
